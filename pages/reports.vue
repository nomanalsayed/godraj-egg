      ...
      <table class="w-full border-collapse">
        ...
        <tbody>
          <tr v-for="report in reports" :key="report.week_date" class="hover:bg-gray-50">
            ...
            <td class="p-2 border">
              {{ report.weekly_production?.toFixed(2) }}
            </td>
            <td class="p-2 border">
              {{ report.adjusted_production?.toFixed(2) }}
            </td>
            <td class="p-2 border">
              {{ report.weekly_depletion?.toFixed(2) }}
            </td>
            <td class="p-2 border">
              {{ report.cumulative_depletion?.toFixed(2) }}
            </td>
          </tr>
        </tbody>
      </table>
...
const { data: reportsData, pending } = await useFetch<{ data: Report[] }>('/api/reports')
