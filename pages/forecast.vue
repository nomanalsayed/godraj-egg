  ...
  <div>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in forecastData" :key="index">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                {{ item.total_birds.toLocaleString() }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ item.avg_age.toFixed(1) }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ (item.production_rate * 100).toFixed(1) }}%
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-blue-600">
                {{ item.forecast_eggs.toLocaleString() }}
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <div v-else class="bg-gray-50 p-12 rounded-lg text-center">
      <p class="text-gray-500 text-lg">
        Set parameters and click "Generate Forecast" to see predictions
      </p>
    </div>
  </div>
</template>
